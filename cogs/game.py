...
class Game(commands.Cog):
    ...
    @app_commands.command(name="game", description="This is for sure broken...")
    async def game(
        self,
        ...
        game: Literal[
            ...
            "Mad Minute",
            "Word guess",
            "I wish to cross the bridge of death",
        ],
    ):
        if game.lower() == "i wish to cross the bridge of death":
            await self.bridge_of_death(interaction)
        elif game.lower() == "mad minute":
            await self.mad_minute(interaction)
        elif game.lower() == "game show":
            await self.game_show(interaction)
        elif game.lower() == "tic tac toe":
            await self.tic_tac_toe(interaction)
        elif game.lower() == "fermi":
            await self.fermi(interaction)
        elif game.lower() == "word guess":
            await self.guess(interaction)
        else:
            return await interaction.response.send_message("You what? Get some /help")

    async def tic_tac_toe(self, interaction):
        client = self.bot
        await interaction.response.send_message("This is an beta test")
        await interaction.channel.send("Who do you want to play against?")
        difficulty = None
        try:
            msg = await client.wait_for(
                "message", check=utils.check(interaction.user), timeout=60
            )
        except asyncio.TimeoutError:
            await interaction.channel.send("Allow me to play the game I select myself")
            msg = None
        if msg:
            ...
            try:
                ...
                await interaction.channel.send(
                    "player not found. Fine I will do it myself"
                )
                player = client.user
                ...
                player = client.user
        if not difficulty:
            await interaction.channel.send("What difficulty?\nEasy\nHard")
            difficulty = "easy"
            try:
                msg = await client.wait_for(
                    "message", check=utils.check(interaction.user), timeout=60
                ...
                await interaction.channel.send("You had one job.")
        ...
        current = ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣"]
        await message.edit(content=board)
        ticker = await interaction.channel.send(f"{interaction.user.mention}'s move")
        ...
        while len(current) != 0:
            await ticker.edit(content=interaction.user.mention + "'s move")
            try:
                # returns a tuple with a reaction and member object
                reaction, member = await client.wait_for(
                    "reaction_add",
                    timeout=30,
                    check=check_reaction(interaction.user.author),
                )
            except asyncio.TimeoutError:
            ...
            if not str(reaction.emoji) in current:
                return await interaction.channel.send("wait that was illegal")
            ...
            if difficulty != "player":
                ...
                try:
                    reaction, member = await client.wait_for(
                        "reaction_add", timeout=30, check=check_reaction(player)
                    )
                except asyncio.TimeoutError:
                    return await ticker.edit(content="The game timed out")
                await ticker.edit(content="Nice choice!")
    ...
    async def fermi(self, interaction):
        client = self.bot
        rounds = 5
        await interaction.response.send_message(
            f"{interaction.user.display_name} Normal(3 digits) or hard(5 digits)?"
        )
        try:
            msg = await client.wait_for(
                "message", check=utils.check(interaction.user), timeout=60
            ...
            await interaction.channel.send("Seems like you want 3")
            size = 3
        if msg and (
            msg.content.lower() == "hard"
            or msg.content.lower() == "h"
            or msg.content.lower() == "5"
        ):
            size = 5
        else:
            size = 3
        ...
        f = Fermi(size)
        await interaction.channel.send(
            f"{interaction.user.display_name} at anytime you can type quit to quit\nWhat is your first guess?"
        )
        try:
            msg = await client.wait_for(
                "message", check=utils.check(interaction.user), timeout=60
            ...
            return await interaction.channel.send("Nothing? Got it.")
        win = f.check_win(msg.content)
        answers = msg.content + " " + win + "\n"
        await interaction.channel.send(win)
        rounds -= 1
        while (rounds > 0 and win != winning) and msg.content.lower() != "quit":
            await interaction.channel.send(
                f"{interaction.user.display_name}\nGuesses:\n{answers}\nWhat is your next guess? (Guesses remaining {rounds}"
            )
            try:
                msg = await client.wait_for(
                    "message", check=utils.check(interaction.user), timeout=60
                ...
                return await interaction.channel.send(
                    "Nothing? Got it. The answer was {}".format(f.get_cheats())
                )
            win = f.check_win(msg.content)
            answers += msg.content + " " + win + "\n"
            rounds -= 1
        if msg.content.lower() == "quit":
            return await interaction.channel.send(
                "~~You did your best~~ You gave up. The answer was {}".format(
                    f.get_cheats()
                )
            )
        if win == winning:
            return await interaction.channel.send("You got it!")
        else:
            return await interaction.channel.send(
                "There is always next time. The answer was {}".format(f.get_cheats())
            )

    async def game_show(self, interaction):
        game = GameShow()
        await interaction.response.send_message("Creating a thread.")
        thread = await interaction.channel.create_thread(
            name="Game show",
            type=discord.ChannelType.public_thread,
            auto_archive_duration=60,
        )
        await game.setup_game(interaction, thread, self.bot)
        await game.run_game(interaction, thread, self.bot)

    async def bridge_of_death(self, interaction):
        client = self.bot
        colors = [
            "red",
            "green",
        ...
        await interaction.channel.send("What is your name?")
        try:
            msg = await client.wait_for(
                "message", check=utils.check(interaction.user), timeout=60
            )
        except asyncio.TimeoutError:
            return await self.failure(interaction)
        try:
            if (
                not msg.content.lower() == interaction.user.name.lower()
                and not msg.content.lower() == interaction.user.nick.lower()
            ):
                return await self.failure(interaction)
        ...
        try:
            ...
            await self.failure(interaction)
            return
        ...
        if q3 == 1:
            ...
            try:
                ...
                return await interaction.channel.send(
                    "I have a bug. Tell my creator\n" + e
                )
            question = ""
            try:
                if api_QA["results"][0]["type"] == "boolean":
                    question += "True or False?\n"
                question += utils.decodeHTMLSymbols(api_QA["results"][0]["question"])
                if api_QA["results"][0]["type"] == "multiple":
                    choices = []
                    choices.append(
                        utils.decodeHTMLSymbols(api_QA["results"][0]["correct_answer"])
                    )
                    ...
                    while len(choices) > 1:
                        ...
                        del choices[i]
                    question += choices[0]
                answer = utils.decodeHTMLSymbols(api_QA["results"][0]["correct_answer"])
            ...
            return await interaction.channel.send(
                "You really should not be seeing this..."
            )
        await interaction.channel.send(f"{name}\n{question}")
        try:
            msg = await client.wait_for(
                "message", check=utils.check(interaction.user), timeout=15
            )
        except asyncio.TimeoutError:
            return await self.failure(interaction)
        if q3 == 4 and str(msg.content).lower() == answer.lower():
            await interaction.channel.send("Huh? I… I don’t know that.")
            return await interaction.channel.send(
                "https://i.makeagif.com/media/8-06-2015/6tzcHH.gif"
            )
        if not str(msg.content).lower() == answer.lower():
            return await self.failure(interaction)
        await interaction.channel.send("Be on your way")
        return await interaction.channel.send(
    ...
    async def mad_minute(self, interaction):
        client = self.bot
        await interaction.response.send_message(
            "You will have 1 minute to answer some multiplication problems. Good luck!"
        )
        await asyncio.sleep(2)
        attempts = 0
        correct = 0
        ...
        while time.time() - start <= 60:
            ...
            await interaction.channel.send(f"What is {x}x{y}?")
            try:
                limit = 60 - (time.time() - start)
                msg = await client.wait_for(
                    "message", check=utils.check(interaction.user), timeout=limit
                )
        ...
        if attempts != 0:
            await interaction.channel.send(
                f"You made {attempts} attempt(s) and got {correct} correct"
            )
        else:
            return await interaction.channel.send("You did nothing.")

    async def failure(self, interaction):
        fails = [
            "https://tenor.com/view/monty-python-bridge-yellow-gif-9412030",
            "https://tenor.com/view/cow-catapult-monty-python-monty-python-and-the-holy-grail-holy-grail-flying-cow-gif-7857398175964679549",
        ]
        await interaction.channel.send(fails[random.randint(0, len(fails) - 1)])
    ...
    async def guess(self, interaction):
        ...
        if win:
            return await interaction.channel.send(
                f"You got the word! You score was {score}"
            )
        else:
            return await interaction.channel.send(
                f"You did your best\nthe word was {word}"
