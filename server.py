...
@app.route("/checkimagejobs/<int:guild_id>")
async def check_for_image_jobs(guild_id):
    threading.Thread(target=update_image_jobs, args=(guild_id,)).start()
    return f"Checks started for{guild_id} image jobs"


@app.route("/doneimagejobs/<int:guild_id>")
async def post_done_image_jobs(guild_id):
    response = await ipc_client.request("post_image_jobs")
    return response.response


@app.route("/check_insta_jobs/<int:guild_id>")
async def check_for_insta_jobs(guild_id):
    threading.Thread(target=update_insta_jobs, args=(guild_id,)).start()
    return f"Checks started for{guild_id} jobs "


@app.route("/done_insta_jobs/<int:guild_id>")
async def post_done_insta_jobs(guild_id):
    response = await ipc_client.request("post_insta_jobs")
    return response.response
...
def update_ai_jobs(guild_id: int):
    ...
    for job in client.Monty.gen_text.find({"server": guild_id, "state": "new"}):
        ...
        models = {
            "mistral": "mistral-7b-instruct-v0.1.Q4_0.gguf",
            "wizardlm": "wizardlm-13b-v1.2.Q4_0.gguf",
            "falcon": "gpt4all-falcon-newbpe-q4_0.gguf",
            "mpt": "mpt-7b-chat-newbpe-q4_0.gguf",
        }
        file_path = os.path.join("/src/bot/models/", models[model])
        chat_model = GPT4All(file_path)
        with chat_model.chat_session():
            response = chat_model.generate(
                prompt=prompt, temp=temperature, n_predict=2048
            )
...
def update_image_jobs(guild_id: int):
    client = MongoClient("mongodb://192.168.1.107:27017/")
    for job in client.Monty.gen_image.find({"server": guild_id, "state": "new"}):
        prompt = job["prompt"]
        ...
        file = f"{id(prompt)}.png"
        image.save(file)
        update = {"$set": {"file": file, "state": "Ready!"}}
        client.Monty.gen_image.update_one(job, update)
        x = requests.get(f"http://192.168.1.107:5101/doneimagejobs/{guild_id}")
        print(x)


def update_insta_jobs(guild_id: int):
    path = "/src/bot/down/"
    client = MongoClient("mongodb://192.168.1.107:27017/")
    new_jobs = False
    tweet = None
    if not os.path.exists(path):
        os.mkdir(path)
    for job in client.Monty.insta_downloader.find({"server": guild_id, "state": "new"}):
        URL = job["URL"]
        new_jobs = True
        has_photo = False
        print(f"starting thread for {URL}")
        ...
        if not os.path.exists(file_path):
            os.mkdir(file_path)
        try:
            video = insta_vid(URL, file_path)
        except InstaDownloadFailedError as e:
            reason = f"Error in download:{e}"
            fail_this_insta_job(client, guild_id, job, reason)
            x = requests.get(f"http://192.168.1.107:5101/done_insta_jobs/{guild_id}")
            print(x)
            continue
        for file in os.listdir(file_path):
            the_file = os.path.join(file_path, file)
        ...
        the_file = os.listdir(file_path)
        update = {"$set": {"state": "Ready!", "file": the_file, "path": file_path}}
        client.Monty.insta_downloader.update_one(job, update)
        print(f"ending thread for {URL}")
        x = requests.get(f"http://192.168.1.107:5101/done_insta_jobs/{guild_id}")
        print(x)
...
def fail_this_insta_job(client, guild_id, job, reason):
    print(reason)
    update = {"$set": {"state": "Failed", "reason": reason}}
    client.Monty.insta_downloader.update_one(job, update)
